...
function removeFirstOccurrences(str, value) {
  const firstOne = str.indexOf(value);
  if (firstOne !== -1) {
    return str.slice(0, firstOne) + str.slice(firstOne + value.length);
  }
  return str;
}
...
function removeLastOccurrences(str, value) {
  const lastOne = str.lastIndexOf(value);
  if (lastOne !== -1) {
    const result = str.slice(0, lastOne) + str.slice(lastOne + value.length);
    return result;
  }
  return str;
}
...
function findLongestWord(sentence) {
  ...
  let result = '';
  for (let i = 0; i < splitedWords.length; i += 1) {
    if (splitedWords[i].length > result.length) {
...
function reverseWords(str) {
  ...
  let reverse = '';
  for (let i = 0; i < splitedWords.length; i += 1) {
    for (let j = splitedWords[i].length - 1; j >= 0; j -= 1) {
      reverse += splitedWords[i][j];
    }
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    } else inverted += char.toUpperCase();
  }
  return inverted;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (char >= 65 && char <= 90) {
      newChar = ((char - 65 + 13) % 26) + 65;
    } else if (char >= 97 && char <= 122) {
      newChar = ((char - 97 + 13) % 26) + 97;
    } else newChar = char;
